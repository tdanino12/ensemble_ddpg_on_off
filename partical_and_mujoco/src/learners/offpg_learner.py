...
class OffPGLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, log):
        ...
        q_vals,q1,q2,q3,q4,q5,q6,q7,q8,q9,q10 = self.critic.forward(inputs[:, :-1], mac_out)
        ...
        coma_loss = -(self.mixer.forward(q_vals.view(actions.shape[0],actions.shape[1],actions.shape[2]), states,False,None)* mask).sum() / mask.sum()
    ...
    def train_critic(self, on_batch, best_batch=None, log=None):
        ...
        target_q_vals,t_q1,t_q2,t_q3,t_q4,t_q5,t_q6,t_q7,t_q8,t_q9,t_q10 = self.target_critic.forward(target_inputs, actions)
        ...
        x = target_q_vals.sum(dim=-1)
        targets_taken = self.target_mixer(x.view(actions.shape[0],actions.shape[1],actions.shape[2]), states, False,None)
        ...
        for t in range(max_t - 1):
            ...
            q_vals,q1,q2,q3,q4,q5,q6,q7,q8,q9,q10  = self.critic.forward(inputs[:, t:t+1], actions[:, t:t+1])
            ...
            x = th.sum(q_vals, dim=-1).view(-1)
            q_vals = self.mixer.forward(x.view(actions[:, t:t+1].shape[0],actions[:, t:t+1].shape[1],actions[:, t:t+1].shape[2]), states[:, t:t+1],False,None)
    ...
    def train_critic_best(self, batch):
        ...
        target_q_vals_new,q1,q2,q3,q4,q5,q6,q7,q8,q9,q10 = self.target_critic.forward(target_inputs, mac_out)
        
        target_q_vals,q1,q2,q3,q4,q5,q6,q7,q8,q9,q10 = self.target_critic.forward(target_inputs, actions)
        target_q_vals = target_q_vals.detach()
        ...
        x = th.sum(target_q_vals_new, dim=-1).view(-1)
        targets_taken = self.target_mixer(x.view(actions.shape[0],actions.shape[1],actions.shape[2]), states,False,None)
        ...
        #expected q
        exp_q = self.build_exp_q(target_q_vals, mac_out, states, actions).detach()
        # td-error
    ...
    def build_exp_q(self, target_q_vals, mac_out, states, actions):
        #th.sum(target_q_vals * mac_out, dim=3)
        target_exp_q_vals = th.sum(target_q_vals, dim=-1).view(-1)
        target_exp_q_vals = self.target_mixer.forward(target_exp_q_vals.view(actions.shape[0],actions.shape[1],actions.shape[2]), states,False,None)
