        ...
        for al_id, al_unit in self.agents.items():
            if not self.death_tracker_ally[al_id]:
                ...
                if al_unit.health == 0:
                    ...
                    # delta_ally += prev_health * neg_scale  # SPARSE (FoX Table 1): drop hit-point bonus on death
                # else:
                #     # still alive
                #     delta_ally += neg_scale * (
                #         prev_health - al_unit.health - al_unit.shield
                #     )  # SPARSE (FoX Table 1): no per-step damage-taken reward
        ...
        for e_id, e_unit in self.enemies.items():
            if not self.death_tracker_enemy[e_id]:
                ...
                if e_unit.health == 0:
                    ...
                    # delta_enemy += prev_health  # SPARSE (FoX Table 1): drop hit-point bonus on kill
                # else:
                #     delta_enemy += prev_health - e_unit.health - e_unit.shield  # SPARSE (FoX Table 1): no per-step damage-dealt reward
